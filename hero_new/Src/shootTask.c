...
static void frictiongearControl(void) {
    ...
    if (gimbal.gimbal_mode == keyboard) {
        ...
        if (rcDevice.rc_info.mouse.l - rcDevice.last_rc_info.mouse.l == -1) {
            pluck_one_flag++;
        }
        if (rcDevice.rc_info.kb.bit.B == true) {
            B_time ++;
            if (B_time >= 270) {
                shoot.frictiongear_flag = false;
                pluck_one_flag = 0;
...
static void pluckControl(void) {
    float position_out;
    if (gimbal.gimbal_mode == remote) {  //遥控模式
        ...
        if ((shoot.frictiongear_flag == true) && (pluck_one_flag >= 2)) {
